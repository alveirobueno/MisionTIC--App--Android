...
import com.example.sitiosmisiontic.ui.DetailActivity.Companion.POI_KEY
...
//        recyclerView.addItemDecoration(decortion)
        ...
        recyclerAdapter.setOnItemClickListener(object : RecyclerViewAdapter.OnItemClickListener {
            override fun onItemClick(position: Int, data: SiteData) {
                Toast.makeText(activity, "Has seleccionado ${data.cityName}", Toast.LENGTH_SHORT)
                    .show()

                val intent = Intent(activity, DetailActivity::class.java)

//                intent.putExtra(CITY_NAME_KEY, data.cityName)
//                intent.putExtra("image_URL", data.imageURL)
